...
(define (f-1-11-rec n)
  (if (<  n 3)
      n
      (+ (f-1-11-rec (- n 1)) (* 2 (f-1-11-rec (- n 2))) (* 3 (f-1-11-rec (- n 3))))))
...
(define (pascal-triangle-num x y)
  (if (or (= x 0) (= y 0)) 1 (+  (pascal-triangle-num (- x 1) y) (pascal-triangle-num x (- y 1)))))
...
#|
Order of grouth of operations number is O(n^2)
Order of grouth of space is O(n)
|#
